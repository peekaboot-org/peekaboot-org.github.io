...
|---|---|---|
| **Fill** | `--pk-primary`, `--pk-warning`, `--pk-info` (and `--pk-success`, `--pk-danger`) | Backgrounds &mdash; badge fills, buttons, banners. Each fill token is paired with an `--pk-on-*` ink token (`--pk-on-primary`, `--pk-on-warning`, `--pk-on-info`, ...) drawn on top of it. |
